...
{
# cat('\nrun git pull...\n')	
# git_pull_out = sprintf('%s pull',git_exe)%>%system(.,intern=TRUE)

if (verbose) git_pull_out%>%paste(.,collapse="\n")%>%cat

(git_add = other_files%>%dplyr::filter(type!="deleted")%>%select(file)%>%first%>%paste0(.,collapse=" ")%>%sprintf("%s add %s",git_exe,.))
# comment = "'this is a R function for pushing git files but leaves the larges files out'"
(git_commit= sprintf("%s commit -m '%s'", git_exe,comment))
(git_push = sprintf("%s push",git_exe))
(git_rm = other_files%>%dplyr::filter(type=="deleted")%>%select(file)%>%first%>%paste(.,collapse=" ")%>%sprintf("%s rm %s",git_exe,.))
cat('\nrun git add...\n')
